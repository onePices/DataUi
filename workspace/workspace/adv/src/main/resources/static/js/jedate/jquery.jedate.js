...
;(function(root, factory) {
    ...
    $.extend({
        jeDate:function(elem, options){
            return $(elem).each(function(){
                options.language = jet.lang((LOCALLANG || 'CN'));
                return new jeDate($(this),options||{});
            });
        }
    });
    ...
    jet.returnDate = function(obj, format, dateval) {
        ...
        var setdate = new Date(narr[0]+parseInt(sparr[0]||00), narr[1]+parseInt(sparr[1]||00)-1, narr[2]+parseInt(sparr[2]||00), narr[3]+parseInt(sparr[3]||00), narr[4]+parseInt(sparr[4]||00), narr[5]+parseInt(sparr[4]||00)),
    ...
    jedfn.createDaysHtml = function(ys, ms, opts, boxCell){
        ...
        boxCell.find(".jedateyear").attr("year", year).text(year+(lang.name == "cn" ? "":""));
        if(lang.monthEn){
            boxCell.find(".jedatemonth").attr("month", month).text(lang.monthEn[month-1]);
        }else{
        ...
        var eachYears = function(YY) {
                ...
                $.each(new Array(15), function(i,v) {
                    if (i === 7) {
                        ...
                        eachStr += "<li " + ycls + ' yy="' + YY + '">' + (YY+(lang.name == "cn" ? "":"")) + "</li>";
                    } else {
                        ycls = (parseInt(YY - 7 + i) >= parseInt(minArr[0]) && parseInt(YY - 7 + i) <= parseInt(maxArr[0])) ? "" : 'class="disabled"';
                        eachStr += '<li ' + ycls + ' yy="' + (YY - 7 + i) + '">' + (YY - 7 + i+(lang.name == "cn" ? "":"")) + "</li>";
                    }
                });
                return eachStr;
            },
            ...
            eachYearMonth =function (YY, ymlen) {
                ...
                if (ymlen == 12) {
                    $.each(lang.month, function(i, val) {
                        ...
                        ymStr += "<li class='"+ mcls +"' mm='" + val + "'>" + (lang.monthEn ? lang.monthEn[i] : (val+(lang.name == "cn" ? "\u6708":""))) + "</li>";
                    });
